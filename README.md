...
| **"Up next"** | The whole catalog bar the video playing, uncapped — scroll far enough and every video the kid may watch is reachable. The list scrolls in its own box (a sidebar from `lg` up, a capped block under the player below that) so an uncapped list can't run the page on forever. Shuffled once per page load and then held fixed: picking a video out of the list re-runs the watch page, but the list comes back in the same order minus the video now playing — a kid working down the list isn't chasing thumbnails that move between taps. A browser reload (or a new tab) deals a new order. |
